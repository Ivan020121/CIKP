...
from stft import TFusion
from mamba import HARMamba
...
def train_model(model, train_loader, val_loader, criterion, optimizer, device='cpu', checkpoint_path: str = "checkpoint"):
    ...
    for epoch in range(train_epoch):
        ...
        # torch.save(model.state_dict(), "imu_weight.pth")
...
train_loader, val_loader, test_loader = get_dataloaders(train_set, val_set, test_set, batch_size=batch_size, data_type='all')

# model = SimpleModel(seq_len=200, n_fft=64, hop_length=8, num_classes=10, device=device, in_channels=9)
# model.init_stft_embedder(train_loader)

# model = HARMamba(seq_size=200, patch_size=16, stride=16, depth=12, num_classes=10, final_pool_type='mean', if_cls_token=False, c_in=9)

model = TFusion(200, 64, 8, device, 9, 16, 16, 12, 10)
model.stft_encoder.init_stft_embedder(train_loader)

criterion = nn.CrossEntropyLoss()
optimizer = optim.Adam(model.parameters(), lr=0.001)

train_model(model, train_loader, test_loader, criterion, optimizer, device=device)
# model.load_state_dict(torch.load("imu_weight.pth"))
# test_acc = evaluate_model(model, "imu_weight", test_loader, device=device)